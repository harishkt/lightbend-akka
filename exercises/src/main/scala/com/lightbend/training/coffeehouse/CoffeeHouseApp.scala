...
object CoffeeHouseApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    implicit val statusTimeout = system.settings.config.getDuration("coffee-house.status-timeout", TimeUnit.MILLISECONDS).millis
...
class CoffeeHouseApp(system: ActorSystem)(implicit statusTimeout: Timeout) extends Terminal {
  ...
  protected def createGuest(count: Int, coffee: Coffee, caffeineLimit: Int = Int.MaxValue): Unit =
    (1 to count) foreach {
  ...
  protected def status(): Unit = {
    (coffeeHouse ? CoffeeHouse.GetStatus).mapTo[CoffeeHouse.Status].onComplete {
      ...
      case Failure(error) => log.error(error, message = "Can't get status!")
    }
  }
}
